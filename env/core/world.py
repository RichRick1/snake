...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = tuple([np.random.randint(SNAKE_SIZE, self.size[0]-SNAKE_SIZE),
                                    np.random.randint(SNAKE_SIZE, self.size[1]-SNAKE_SIZE)])
            # choose a random direction index
            start_direction_index = np.random.randint(4)
    ...
    def init_food(self):
        ...
        tmp = self.world.copy()
        tmp[tuple(zip(*self.snake.blocks))] = 1
        available_food_positions = list(zip(*np.where(tmp == 0)))
        if not self.custom:
            # Choose a random position from available
            chosen_position = available_food_positions[np.random.randint(len(available_food_positions))]
        else:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if new_snake_head in set(zip(*np.where(self.world == WALL))):
